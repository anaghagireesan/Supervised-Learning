...
def dynamic_decode(decoder,
  ...
  with variable_scope.variable_scope(scope, "decoder") as varscope:
    ...
    mec_attr = tensorflow.ones(shape=[batch,embed_vocab,num_attr],dtype=dtypes.int32)
    initial_finished, initial_inputs, initial_state,mec_attr  = decoder.initialize()
    ...
    logits = tensorflow.zeros(shape=[batch,1,vocab], dtype=dtypes.float32)
    z = tensorflow.ones(shape=[batch,1,vocab], dtype=dtypes.float32)
    ...
    '''def condition(unused_time, unused_outputs_ta, unused_state, unused_inputs,
                  finished, unused_sequence_lengths,logits):'''
    def condition(unused_time, unused_outputs_ta, unused_state, unused_inputs,
                  finished, unused_sequence_lengths,z,mec_attr,logits):
      return math_ops.logical_not(math_ops.reduce_all(finished))

    #def body(time, outputs_ta, state, inputs, finished, sequence_lengths,logits):
    def body(time, outputs_ta, state, inputs, finished, sequence_lengths,z,mec_attr,logits):
      ...
      '''(next_outputs, decoder_state, next_inputs,
       decoder_finished,logits) = decoder.step(time, inputs, state)'''
      (next_outputs, decoder_state, next_inputs,
       decoder_finished,z,mec_attr,logits) = decoder.step(time, inputs, state, mec_attr)
      ...
      '''return (time + 1, outputs_ta, next_state, next_inputs, next_finished,
              next_sequence_lengths,logits)'''
      return (time + 1, outputs_ta, next_state, next_inputs, next_finished,
              next_sequence_lengths,z,mec_attr,logits)
    ...
    res = control_flow_ops.while_loop(
        ...
        #loop_vars=[
            ...
            #initial_finished, initial_sequence_lengths,logits,
        #],
        loop_vars=[
            initial_time, initial_outputs_ta, initial_state, initial_inputs,
            initial_finished, initial_sequence_lengths,z,mec_attr,logits
        ],
  ...
  return final_outputs, final_state, final_sequence_lengths,z,mec_attr,logits,time
