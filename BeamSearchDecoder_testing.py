...
def _check_static_batch_beam_maybe(shape, batch_size, beam_width):
  ...
  """
  #print("_check_static_batch_beam_maybe")
...
class BeamSearchDecoder(decoder.Decoder):
  ...
  def __init__(self,
    ...
    self._finished = array_ops.one_hot(
        ...
        off_value=True,
        dtype=dtypes.bool)
    #print("self finished", self._finished)
  ...
  def initialize(self, name=None):
    ...
    return (finished, start_inputs, initial_state,self._mec_attr)
  ...
  def step(self, time, inputs, state,mec_attr, name=None):
    ...
    with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      current_user_attr = tf.zeros(shape=[batch,num_attr],dtype=tf.int32)
      ...
      def body(ideal,z,cell_outputs,ids,current_user_attr,temp):
        ...
        diff_bool = tf.math.greater_equal(current_base_attr,current_user_attr)
        ...
        x = tf.sparse_to_dense(sparse_indices=ind,
                           output_shape=[batch, vocab],
                           default_value=1,
                           sparse_values=0,)
        z = tf.cast(tf.bitwise.bitwise_or(x,diff_int),dtype=tf.float32)
        z = tf.where(tf.equal(z, 0), tf.constant(-20,shape=[batch,vocab],dtype=tf.float32), z)
      ...
      def condition(ideal,z,cell_outputs,ids,current_user_attr,temp):
        #return tf.less(temp,tf.constant(1,dtype=tf.int32))
        return tf.logical_not(tf.reduce_all(tf.equal(ideal,z)))

      ideal,z,cell_outputs,ids,current_user_attr,temp = tf.while_loop(condition, body, loop_vars=[ideal,z,cell_outputs,ids,current_user_attr,temp],
                      shape_invariants=[ideal.get_shape(),tf.TensorShape(None),
                                       cell_outputs.get_shape(),tf.TensorShape(None),
                                        tf.TensorShape(None),temp.get_shape()
                                       ])
      ...
      def pass_val(cell_outputs):
          return cell_outputs
      cell_outputs = tf.cond(tf.equal(time,end), lambda: end_prediction(cell_outputs), lambda: pass_val(cell_outputs))
    ...
    return (beam_search_output, beam_search_state, next_inputs, finished,z,mec_attr,cell_outputs)
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
  ...
  return output, next_state ,word_indices
...
def _maybe_tensor_gather_helper(gather_indices, gather_from, batch_size,
  ...
  """
  #print("_maybe_tensor_gather_helper")
